...
install_v2node() {
    ...
    if  [[ -z "$version_param" ]] ; then
        last_version=$(curl -Ls "https://api.github.com/repos/besnow/v2node/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    ...
    last_version=$version_param
        url="https://github.com/besnow/v2node/releases/download/${last_version}/v2node-linux-${arch}.zip"
        curl -sL "$url" | pv -s 30M -W -N "下载进度" > /usr/local/v2node/v2node-linux.zip
        if [[ $? -ne 0 ]]; then
...
EOF
    ...
    curl -o /usr/bin/v2node -Ls https://raw.githubusercontent.com/besnow/v2node/main/script/v2node.sh
